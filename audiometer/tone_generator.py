...
class ToneThread(threading.Thread):
    ...
    def make_stereo_tone(self):
        ...
        left_wave = (
            audio_wave
            if self.channel in (self.LEFT_CHANNEL, self.BOTH_CHANNELS)
            else empty_wave
        )
        right_wave = (
            audio_wave
            if self.channel in (self.RIGHT_CHANNEL, self.BOTH_CHANNELS)
            else empty_wave
        )
    ...
    def generate_sine_tone(self):
        ...
        signal = (
            max_sample * self.volume * np.sin(2 * np.pi * self.frequency * time_vector)
        )
        return signal
