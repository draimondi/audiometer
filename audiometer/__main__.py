...
def main():
    ...
    while True:
        ...
        if event in {"program_launch", "stop"}:
            ...
            window = sg.Window(
                PROGRAM_TITLE, default_layout, grab_anywhere=True, finalize=True
            )
            window.Refresh()
        ...
        if event == "start":
            ...
            window = sg.Window(
                PROGRAM_TITLE, test_layout, grab_anywhere=True, finalize=True
            )
            window.Refresh()
        ...
        if event in {"right", "left"}:
            if exam.sound_is_heard(event):
                window["output"].update(
                    f"Sound correctly detected from the {event}", text_color="green"
                )
            else:
                window["output"].update(
                    f"Incorrect. Sound is not coming from the {event}",
                    text_color="red",
                )
